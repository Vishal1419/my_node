...
module.exports = {
    ...
    getRestaurantListForNotiGrp:function (request, response) {
        var vccResponse = new VccResponse(response);
        var pageNo = request.body.page_no;
        var noOfRecords = request.body.no_of_records;
        var startIndex = 0;
        var whereCondition = "WHERE active = :active ";

        if(Utility.isNull(request.body.restaurant_name) === false &&
            validate.isEmpty(request.body.restaurant_name) === false){
            whereCondition = whereCondition + " AND restaurants.name LIKE :restaurant_name ";
        }

        if(Utility.isNull(request.body.city_area) === false &&
            validate.isEmpty(request.body.city_area) === false){
            whereCondition = whereCondition + " AND restaurants.city_area=:city_area ";
        }

        if(Utility.isNull(request.body.city) === false &&
            validate.isEmpty(request.body.city) === false){
            whereCondition = whereCondition + " AND restaurants.city=:city ";
        }
            ...
            [
                function (callback) {
                    if(pageNo === 1){
                        ...
                        ).then(function (recordCountList) {
                            var recordCountDetails = recordCountList[0];
                            var totalPages = Math.ceil(recordCountDetails.record_count/noOfRecords);
                            callback(null,totalPages,recordCountDetails.record_count);
                        }).catch(function (error) {
                            callback(error);
                        });
                    }else{
                ...
                function (totalPages,recordCount,callback) {
                    startIndex = (pageNo - 1) * noOfRecords;
                    var columns = [
                        "restaurants.id as restaurant_id",
                        "restaurants.name",
                        "restaurants.address",
                        "restaurants.city",
                        "restaurants.city_area",
                        "restaurants.noti_delay_group_id",
                        "noti_delay_group.name AS group_name"
                    ];

                    var query = "SELECT " +
                            columns.join(", ")+ " " +
                        "FROM restaurants " +
                        "LEFT OUTER JOIN noti_delay_group ON restaurants.noti_delay_group_id = noti_delay_group.id "+
                        whereCondition +
                    ...
                    models.sequelize.query(query,
                        ...
                        if(totalPages !== null){
                            result.total_records=recordCount;
                            result.total_pages = totalPages;
                        }

                        if(restaurantList.length === (parseInt(noOfRecords) + 1)){
                            nextPage = true;
                            restaurantList.pop();
                        }

                        result.list = restaurantList;
    ...
    assignNotiDelayGrp:function (request, response) {
        var vccResponse = new VccResponse(response);
            ...
            [
                ...
                function (groupCount,callback) {
                    if(groupCount>0){
                        models.Restaurants.count({
                            ...
                            callback(null,{group_count:groupCount,restaurant_count:count});
                        }).catch(function (error) {
                            callback(error);
                        });
                    }else{
                        console.log(callback);
                        callback(null,{group_count:groupCount,restaurant_count:0});
                    }
                }
                ...
                if(error){
                    ...
                    if(results.group_count>0){
                        if(results.restaurant_count === request.body.restaurant_ids.length){
                            models.Restaurants.update(
                                {
                                    noti_delay_group_id:request.body.group_id
                                },
                                {
                                    fields:[
                                        "noti_delay_group_id"
                            ...
                            vccResponse.setStatusCode(VccResponse.RECORD_NOT_EXIST)
                                .setResponseBody({error: "One or more restaurants not found"})
                                .send();
                        }
                    }else{
                        vccResponse.setStatusCode(VccResponse.RECORD_NOT_EXIST)
                            .setResponseBody({error: "Group not found"})
                            .send();
                    }
                }
            }
