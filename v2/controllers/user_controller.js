...
module.exports = {
    ...
    sendInvite:function (request, response) {
        ...
        var makeFriendToExistingUser = function (invitedFriend,customer) {
            ...
            async.waterfall([
                ...
                function (count1, callback) {
                    models.CustomerInvites.count({
                        where:{
                            customer_id:invitedFriend.id,
                            friend_id:request.token.customer_id
                        }
                    }).then(function (count) {
                        callback(null,count1,count);
                    }).catch(function (error) {
                        Logger.logDbError(error,request);
                        callback(error);
                    });
                },
                function (count1, count2, callback) {
                    var addDate = moment.utc();
                    if(count1<=0){
                        var friendArray = [
                            {
                        ...
                        if(count2<=0){
                            friendArray.push({
                                ...
                                friend_first_name:customer.first_name,
                                friend_last_name:customer.last_name,
                                date_added:addDate,
                                medium:request.body.medium,
                                influence:1,
                                friends_influence:1,
                                hash:crypto.createHash('md5').update((customer.email + moment().utc().unix())).digest("hex")
                            });
                        }

                        models.CustomerInvites.bulkCreate(friendArray,{individualHooks: true})
                            .then(function (results) {
                                callback(null,{count1:count1,count:count2,invitedFriend:invitedFriend,customer:customer});
                            }).catch(function (error) {
                                callback(error);
                        });
                    }else{
            ...
            models.Customers.findOne({
                ...
                async.series({
                        ...
                        invitedTemplate.render(inviteDetail, function (err, results) {
                            if (err) {
                                ...
                                var mailOptions = {
                                    ...
                                    subject: 'You are connected with '+inviteCustomer.first_name+' on VinCompass',
                                    html: results.html
                                };
                    ...
                    sendInviteEmail:function (callback) {
                        var templateDir = path.join(appRoot, 'email_templates', 'invitation','invite');

                        var inviteTemplate = new EmailTemplate(templateDir);
                        var invitedDetail = {
                            to_user_first_name:inviteCustomer.first_name,
                            from_user_full_name: (invitedFriend.first_name + " " +invitedFriend.last_name),
                            from_user_first_name: invitedFriend.first_name
                        };
                        ...
                        inviteTemplate.render(invitedDetail, function (err, results) {
                            if (err) {
                                ...
                                var mailOptions = {
                                    ...
                                    subject: 'You are connected with '+invitedFriend.first_name+' on VinCompass',
                                    html: results.html
                                };
        ...
        async.waterfall([
            ...
            function (invitedFriend, callback) {
                models.Customers.findOne({
                    where:{
                        email:request.token.email
                    }
                }).then(function (customer) {
                    callback(null,{invited_friend:invitedFriend,customer:customer});
                }).catch(function (error) {
                    callback(error);
                })
            }
            ...
            if(error){
                ...
                if(results.invited_friend){
                    makeFriendToExistingUser(results.invited_friend,results.customer);
                }else{
                    insertInviteCode();
                }
            }
        ...
        async.waterfall([
            function (callback) {
                Memcache.sharedInstance()
                    ...
                    .then(function (userTokenArray) {
                        if(userTokenArray){
                            var token = jwt.sign(request.token,constants.TOKEN_ENCRYPTION_KEY);
                            var index = userTokenArray.indexOf(token);
                            if(index !== -1){
                                userTokenArray.splice(index,1);
                                Memcache.sharedInstance()
                                    .getMemcachePlus()
                                    .set(request.token.email,userTokenArray)
                                    .then(function () {
                                        callback(null,true);
                                    }).catch(function (error) {
                                        callback(error);
            ...
            function (tokenExist, callback) {
                if(Utility.isNull(request.body) === false &&
                    Utility.isNull(request.body.device_token) === false){
                    models.CustomerDeviceTokens.update({
                        enable:false,
